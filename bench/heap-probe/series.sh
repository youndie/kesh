...
#
#   bench/heap-probe/series.sh OUT "arm encoding scale repeat" ...
...
declare -A BINARY
for spec in "$@"; do
  read -r arm encoding scale repeat <<<"$spec"
  ...
  base="$OUT/$arm-$encoding-$scale-r$repeat"
  ...
  "${BINARY[$arm]}" --encoding "$encoding" --scale "$scale" --seconds 60 >"$base.out" 2>"$base.err"
  python3 bench/heap-probe/pauses.py "$base.out" "$base.err" --label "$arm $encoding $scale r$repeat" | tail -1
done
